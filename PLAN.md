...
**Bottom line:** one new public repo (`Thalermark/thalermark-site`) holding one
Astro site: the marketing landing page at `thalermark.com` (ported nearly
verbatim, CTAs flipped from "early access" to live) and a Starlight docs site
at `thalermark.com/docs`. Docs are written fresh, jargon-free, as markdown
files, which is also what agents read. Built output is plain static files,
uploaded to the current shared host.
...
- Style: **no em dashes, ever** (user directive 2026-08-13); no accounting
  jargon in docs
...
- Starlight is a plugin inside a normal Astro project: docs get the full docs
  treatment (sidebar, search, dark mode), while `src/pages/index.astro` is a
  free canvas. The existing landing HTML drops in with its inline CSS
  untouched.
- Markdown-in-git satisfies "agents can read it" natively: the source files
  are public on GitHub, and we additionally serve raw `.md` + an `llms.txt`
  index from the site itself (Phase 3).
- Static output means the shared host needs nothing but FTP/SFTP. No Node on
  the server, no database, no new service to babysit.
...
Source: `thalermark/spikes/thalermark-landing.html` (1,162 lines,
self-contained).
...
- **The "early access to live" copy changes (the minor changes requested):**
  - Nav CTA `Get early access` becomes **`Open the app`** linking to
    `https://app.thalermark.com`
  - Hero email form becomes a primary **`Start free`** button to the app; keep
    the `preview@thalermark.com` mailto as a secondary "questions?" link
  - Final CTA eyebrow `Coming soon` becomes **`Now in open beta`**
  - Final CTA form gets the same `Start free` button treatment
  - Footer `Documentation` link: GitHub README becomes **`/docs`**
...
- **Everyday use**: task-titled, mirrors app language (see Status; 19 guides
  shipped, list finalized against the real app)
...
categories, matching things up*. The docs use the same words the app screens
use. When in doubt, quote the UI.

Also banned everywhere on the site: em dashes.
...
against the app or the code, not against aspirational specs.
...
- Serve from the site as well:
  - `llms.txt` (index) and `llms-full.txt` (concatenated) at the site root.
    The `starlight-llms-txt` plugin does this (verified working).
  - Raw per-page markdown at the page URL + `.md`
    (`/docs/invoices` → `/docs/invoices.md`) via a small build step that copies
    content files into `dist/`. Still to do.
- Acceptance: `curl` any docs URL with `.md` and get clean markdown.

## Phase 4: Deploy to the shared host

- `pnpm build` produces `dist/` of static files; upload replaces the current
  site root (landing at `/`, docs at `/docs/`).
- Pipeline, either/both:
  1. **GitHub Action on push to main** with FTP/SFTP upload (e.g.
     `SamKirkland/FTP-Deploy-Action`); host credentials as repo secrets.
  2. **Local script** (`pnpm build && rsync/lftp ...`) if keeping host creds
     off GitHub is preferred.
...
- Core repo follow-ups (for the code-side agent or a small PR):
...
- [x] **Signup reality check:** confirmed open self-serve signup; CTAs link
      `app.thalermark.com/sign-up` / `/sign-in` (routes verified in the web
      app).
...
- [x] "Everyday use" section: 19 guides written 2026-08-13, grounded in UI
      strings extracted from the live web app's Svelte routes (three research
      passes over apps/web). Covers dashboard, invoicing (send / get paid /
      fix / estimates / repeating / card payments), money out (expenses, big
      purchases, bills, owner money), contacts, items, jobs, mileage, reports
...
- Blog / changelog (easy to add later as an Astro content collection).
